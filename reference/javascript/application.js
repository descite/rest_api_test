...
}).call(this);/*! highlight.js v9.3.0 | BSD3 License | git.io/hljslicense */
...
(function() {
  ...
  $(function() {
    var OPEN_LIST_KEY, klass, openList;
    OPEN_LIST_KEY = 'open_list';
    openList = function(klass) {
      $('.toggled').removeClass('visible');
      $(".toggled." + klass).addClass('visible');
      return sessionStorage.setItem(OPEN_LIST_KEY, klass);
    };
    if (klass = sessionStorage.getItem(OPEN_LIST_KEY)) {
      openList(klass);
    }
    window.openClassList = function() {
      return openList('class_list');
    };
    window.openMethodList = function() {
      return openList('method_list');
    };
    window.openMixinList = function() {
      return openList('mixin_list');
    };
    window.openExtraList = function() {
      return openList('extra_list');
    };
    window.createStripes = function() {
      return $('.list li').each(function(i, el) {
        if (i % 2 === 0) {
  ...
  $(function() {
    ...
    key('l, shift+l', function() {
      if ($('body').hasClass(HIDE_SIDEBAR_CLASS)) {
        $('body').removeClass(HIDE_SIDEBAR_CLASS);
        return sessionStorage.removeItem(SIDEBAR_HIDDEN_KEY);
      } else {
        $('body').addClass(HIDE_SIDEBAR_CLASS);
        return sessionStorage.setItem(SIDEBAR_HIDDEN_KEY, 'true');
      }
    });
    if (sessionStorage.getItem(SIDEBAR_HIDDEN_KEY) != null) {
      $('body').addClass(HIDE_SIDEBAR_CLASS);
    }
    ...
    key('h, shift+h', function() {
      return $('#help').toggle();
    });
    return key('t, shift+t', function(e) {
      e.preventDefault();
      $('#fuzzySearch').toggle();
      return $('#fuzzySearch input').focus().select();
    });
  });
